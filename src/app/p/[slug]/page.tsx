...
export default async function ProspectPage({
  ...
  return (
    <main className="bg-white text-[#1a1a1a]">
      ...
      <section className="bg-[#0a0a0a] text-white px-6 py-24">
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-bold leading-tight mb-6">
            {g.label} bei {p.company}: jede Mehrleistung abgerechnet, jede Stunde belegt.
          </h1>
          <p className="text-lg text-gray-300 mb-8 max-w-3xl">{p.opener}</p>
      ...
      {/* What we noticed about this specific firm */}
      {p.facts && p.facts.length > 0 && (
        <section className="px-6 py-16 bg-white border-b border-[#e8e6e1]">
          <div className="max-w-4xl mx-auto">
            <div className="text-sm uppercase tracking-widest text-gray-500 mb-3">
              Das ist uns an {p.company} aufgefallen
            </div>
            <ul className="space-y-3">
              {p.facts.map((f, i) => (
                <li key={i} className="flex gap-3 text-lg text-gray-800">
            ...
            {(p.painpoints && p.painpoints.length > 0
              ...
              .map((pp, i) => (
                <div key={i} className="rounded-xl border border-[#e8e6e1] bg-white p-6">
                  ...
                  <p className="text-gray-800 leading-relaxed">{pp}</p>
                </div>
              ))}
          </div>
        </div>
      ...
      {/* Solutions: aufmass, nachtraege, termine, doku */}
      {[s.aufmass, s.nachtraege, s.termine, s.doku].map((story, idx) => (
        <section key={idx} className={`px-6 py-20 ${idx % 2 === 0 ? "bg-[#f5f0f0]" : "bg-white"}`}>
          <div className="max-w-5xl mx-auto">
